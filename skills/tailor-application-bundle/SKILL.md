...
Require an approved catalog resolved from
`<state-root>/profiles/current.json` by the live
`$maintain-master-curriculum` resolver. Its source binding must match the exact
canonical sources used by candidate evidence. Never use a stale, staged,
unapproved, or job-generated catalog.
...
5. If no profile passes the anchor/support/job-evidence gates, do not write a bundle. Write and validate `profile-proposal.json` using the live `$maintain-master-curriculum` template and validator, explain why a new profile is needed, return `needs_input`, and stop before review, rendering, promotion, or Notion.
...
8. Render work only from typed experience records and place the legal employer or contracting party in the company field; show a client only as a labeled client project. Render education only from typed education records, copying official degree and field. Copy every candidate-evidence warning unchanged into `match_analysis.credibility_warnings`.
9. Draft for one page. Prohibit invented metrics, seniority, tools, dates, relationships, credentials, language proficiency, and domain experience.
10. Run `scripts/validate_bundle.py --bundle <bundle.json>` only for structural integrity: schema, hashes, source references, evidence partition completeness, and citation bookkeeping. Send exact errors back to the same agent once. Exit `0` means structurally ready for agent review, not semantically accepted.
