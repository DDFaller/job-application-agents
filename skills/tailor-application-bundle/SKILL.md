...
7. Draft for one full page using the preservation and page-budget heuristics in `references/job-driven-tailoring.md`. Select concise, job-relevant content while retaining compact baseline context. If the first render is visibly underfilled, expand once using supported evidence in this order: relevant work experience, relevant education, relevant languages/certifications, then other job-relevant facts. Promote any newly used fact from deprioritized to selected evidence and update every affected citation and partition. If overfull, compress repeated wording and optional detail before removing baseline context. Never add irrelevant history merely to occupy space.
